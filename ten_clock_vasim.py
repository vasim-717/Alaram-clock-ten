...
root = Tk()
root.title("Vasim Alarm clock for TEN")
root.geometry("350x200+510+200")
...
def SubmitButton():
    ...
    if AlarmTime == CurrentTime:
        ...
        messagebox.showinfo(title='Alarm Message',
                            message="{}".format(entry2.get()))


def Message1():
    AlarmTimeLable = entry1.get()
    label2.config(text="the Alarm time is Counting...")
    messagebox.showinfo(title='Alarm clock',
                        message='Alarm will Ring at {}'.format(AlarmTimeLable))
...
entry1.pack()
entry1.insert(3, "Type here  (Ex-18:15)")
...
button1 = ttk.Button(frame1,
                     text="submit", command=SubmitButton)
button1.pack()
